...
if test $# -lt 1
then
  echo "== Usage: $0 [start|stop|check|mkdir|rm|ls|classpath|ldpath] <hdfs_path>"
  exit 1
fi

me=$0
INDEXFS_HOME=$(cd -P -- `dirname $me`/.. && pwd -P)
HADOOP_CONF_DIR=$INDEXFS_HOME/etc/hdfs-standalone
export HADOOP_CONF_DIR=$HADOOP_CONF_DIR

# check JAVA_HOME configuration
if test "x$JAVA_HOME" = "x"
then
  for java_base in /usr/local/sunjdk /opt/sunjdk $HOME/usr/sunjdk
  do
    if test -e $java_base/jre/bin/java
    then
      JAVA_HOME=$java_base
    fi
  done
...
then
  for hadoop_base in /usr/local/hadoop2 /opt/hadoop2 $HOME/usr/hadoop2
  do
...
mkdir -p $INDEXFS_HADOOP_RUN $INDEXFS_HADOOP_LOGS \
  $INDEXFS_HADOOP_NAME_DIR $INDEXFS_HADOOP_DATA_DIR
}
...
for hdfs_proc in "namenode" "datanode"
do
...
##### HDFS Utilities - Tested with Hadoop 2.4.1 #####
exit 0
