...
static
IOClient* CreateIOClient(int my_rank) {
  if (FLAGS_fs == "indexfs")
    return IOClientFactory::GetIndexFSClient(my_rank, FLAGS_run_id);
...
IOTask::IOTask(int my_rank, int comm_sz)
  : my_rank_(my_rank), comm_sz_(comm_sz) {
  srand(GetRandomSeed());
  LOG_ = OpenLogFile(my_rank);
  IO_ = CreateIOClient(my_rank);
}
